...
<template>
    <NuxtLink v-if="props.status !== 'pending'" :to="`/classroom/${props.class_module}/${props.id}`"
        ...
        <Icon v-if="props.status === 'completed'" class="text-green-500 font-bold text-3xl"
            name="solar:check-read-outline" />

        <Icon v-if="props.status === 'active'" class="text-blue-500 text-2xl animate-pulse"
    ...
    <div v-else
        ...
        <Icon v-if="props.status === 'pending'" class="text-orange-500 text-2xl" name="solar:alarm-line-duotone" />
