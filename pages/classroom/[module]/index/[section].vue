...
const {
    data: activeSection,
    pending: activeSectionPending,
    execute: executeActiveSection,
} = useLazyAsyncData(
    "activeSection",
    () =>
...
const {
    ...
    execute: executeModules,
} = useLazyAsyncData("module", () => ModuleService().getModules(), {
    immediate: false,
});
...
const {
    ...
    pending: UserPending,
    execute: executeUser,
} = useLazyAsyncData("User", () => MetaDataService().getMetaData(), {
    immediate: false,
});
...
const next = () => {
    if (videoData.value?.next_section_id)
        navigateTo(
            `/classroom/${route.params.module}/${videoData.value.next_section_id}`,
        );
};

const nextModule = async () => {
    const { data: nextModule, error } = await useAsyncData("nextModule", () =>
        ModuleService().getNextModule(route.params.module as string),
    );
    ...
    const { data: updatedUser, error: updateUserError } = await useAsyncData(
        "UpdatedUser",
        () =>
            MetaDataService().updateMetaData({
                video_timestamp: 0,
...
<template>
    ...
    >
        <ClassroomVideoPlayer
            :activeSectionStatus="activeSection?.status!"
            @updateTimestamp="updateTimestamp"
            @videoEnded="videoEnded"
            :timestamp="User?.video_timestamp!"
            :options="options"
        />
    </AspectRatio>
    ...
    <nav class="mt-5" v-if="showNav">
        <Button
            v-if="videoData?.next_section_id !== videoData?.section_id"
            @click="next()"
            >Next Section</Button
        >
